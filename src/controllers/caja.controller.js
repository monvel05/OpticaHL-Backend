...
const procesarPago = async (req, res) => {
  ...
  try {
    ...
    const [ordenes] = await connection.query(
      `SELECT o.total, o.estatus, c.email, c.nombre_completo 
     FROM orden o 
     JOIN clientes c ON o.id_cliente = c.id_cliente 
     WHERE o.folio = ? FOR UPDATE`,
      [folio],
    );
    ...
    if (pagoIngresado > saldoActual) {
      throw new Error(
        `El monto a pagar (${pagoIngresado}) es mayor al saldo pendiente (${saldoActual}).`,
      );
    }
    ...
    await connection.query(`UPDATE orden SET estatus = ? WHERE folio = ?`, [
      nuevoEstatus,
      folio,
    ]);
